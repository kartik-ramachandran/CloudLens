...
public class AuthService : IAuthService
{
    ...
    public string GenerateJwtToken(User user)
    {
        ...
        var token = new JwtSecurityToken(
            issuer: _configuration["Jwt:Issuer"] ?? "AzureLens",
            audience: _configuration["Jwt:Audience"] ?? "AzureLens",
            claims: claims,
        ...
        Data.Entities.SsoProviderConfig config, string code, string? codeVerifier, string redirectUri)
    {
        try
        {
            ...
            {
                "microsoft" => $"{config.Authority?.TrimEnd('/')}/oauth2/v2.0/token",
                "google"    => "https://oauth2.googleapis.com/token",
                "okta"      => $"{config.Authority?.TrimEnd('/')}/v1/token",
                _           => $"{config.Authority?.TrimEnd('/')}/token"  // Ping / generic OIDC
            };
