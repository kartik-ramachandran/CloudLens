...
import {
  ...
  AppSecretsReport, KeyVaultExpiryReport,
} from '../types';
...
export const registerWithEmail = async (name: string, email: string, password: string, organizationName?: string): Promise<LoginResponse> => {
  const response = await api.post('/auth/register', { name, email, password, organizationName });
  return response.data;
};
...
// ── Secrets Monitor APIs ──────────────────────────────────────────────────────

export const getAppSecretsReport = async (credentials: AzureCredentials): Promise<AppSecretsReport> => {
  const response = await api.post('/SecretsMonitor/app-secrets', {
    sessionId: credentials.sessionId,
    subscriptionIds: credentials.subscriptionIds,
  });
  return response.data;
...
export const getKeyVaultExpiryReport = async (credentials: AzureCredentials): Promise<KeyVaultExpiryReport> => {
  const response = await api.post('/SecretsMonitor/keyvault-expiry', {
    sessionId: credentials.sessionId,
    subscriptionIds: credentials.subscriptionIds,
  });
  return response.data;
