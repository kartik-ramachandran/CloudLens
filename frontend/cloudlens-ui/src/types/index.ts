...
export interface AppSecretInfo {
  ...
  status: 'expired' | 'expiring_soon' | 'expiring_90d' | 'healthy' | 'no_expiry';
}

export interface AppSecretsReport {
  totalApps: number;
  totalSecrets: number;
  expiredSecrets: number;
  expiringSoon30d: number;
  expiringSoon90d: number;
  healthySecrets: number;
  secrets: AppSecretInfo[];
  generatedAt: string;
}

export interface KeyVaultExpiryItem {
  vaultName: string;
  vaultId: string;
  resourceGroup: string;
  subscriptionId: string;
  itemName: string;
  itemType: 'secret' | 'certificate';
  expiryDate?: string;
  daysUntilExpiry: number;
  status: 'expired' | 'expiring_soon' | 'expiring_90d' | 'healthy' | 'no_expiry';
}

export interface KeyVaultExpiryReport {
  totalVaults: number;
  totalItems: number;
  expiredItems: number;
  expiringSoon30d: number;
  expiringSoon90d: number;
  healthyItems: number;
  noExpiryItems: number;
  items: KeyVaultExpiryItem[];
  generatedAt: string;
}
