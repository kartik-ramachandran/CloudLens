...
import { getAuthProviders, demoLogin } from '../services/api';
...
const SsoLoginPage: React.FC = () => {
  ...
  const handleDemoLogin = async () => {
    ...
    try {
      ...
      if (!result.success || !result.token || !result.user) {
        setError(result.error ?? 'Demo login failed. Is the backend running?');
        return;
      }
      storeAuth(result.token, result.user);
      window.location.reload();
    } catch (e: any) {
      setError(e.response?.data?.error ?? e.message ?? 'Could not reach the backend. Make sure the API is running.');
    } finally {
  ...
  return (
    ...
    >
      {/* ── Main login card ── */}
      <Card sx={{ width: '100%', maxWidth: 440, borderRadius: 3, boxShadow: '0 24px 64px rgba(0,0,0,0.25)' }}>
        <CardContent sx={{ p: { xs: 3, sm: 4 } }}>

          {/* Logo + brand */}
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', mb: 3.5 }}>
            <Avatar sx={{ bgcolor: 'primary.main', width: 56, height: 56, mb: 1.5, boxShadow: '0 4px 14px rgba(102,126,234,0.5)' }}>
          ...
          {loadingProviders ? (
            ...
            <>
              ...
              <Button
                fullWidth
                variant={providers.length === 0 ? 'contained' : 'outlined'}
                color="primary"
                size="large"
                disabled={demoLoading}
                onClick={handleDemoLogin}
                startIcon={demoLoading ? <CircularProgress size={18} color="inherit" /> : <PlayArrowIcon />}
                sx={{ py: 1.4, fontWeight: 600, fontSize: '0.9rem' }}
              >
                {demoLoading ? 'Signing in…' : 'Sign In (Demo Mode)'}
              </Button>
      ...
      >
        ...
        <Alert severity="info" sx={{ mx: 3, mb: 1, fontSize: '0.8rem' }}>
          ...
          The <em>Demo Mode</em> user is auto-granted Admin — sign in with it first to configure your provider.
        </Alert>
        ...
        <DialogActions sx={{ px: 3, pb: 2.5, flexDirection: 'column', alignItems: 'stretch', gap: 1 }}>
          <Alert severity="warning" sx={{ fontSize: '0.78rem' }}>
            <strong>Before going to production:</strong> remove or disable the{' '}
            <code>POST /api/auth/demo-login</code> endpoint in <code>AuthController.cs</code>.
          </Alert>
